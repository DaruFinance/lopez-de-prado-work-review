...
# --- Auxiliary perpetual feeds (cross-sectional study and its audit) ----------
# These back the 787-pair cross-sectional rotation and its forensic audit. Each is
# produced by a separate collection pipeline and is not bundled here; point the
# root at your own copy. Scripts read them lazily and skip a feed if it is absent.
CRYPTO_FUNDING = _root("LDP_CRYPTO_FUNDING", "crypto_funding")            # 8h funding history per pair
CRYPTO_OI = _root("LDP_CRYPTO_OI", "crypto_oi")                          # open-interest history per pair
CRYPTO_ORDERFLOW = _root("LDP_CRYPTO_ORDERFLOW", "crypto_orderflow")     # taker buy/sell flow per pair
CRYPTO_PERP_CM_1H = _root("LDP_CRYPTO_PERP_CM_1H", "crypto_perp_cm_1h")  # COIN-M inverse perps, *_1h.parquet
CRYPTO_DELISTED_FUNDING = _root("LDP_CRYPTO_DELISTED_FUNDING", "crypto_delisted_funding")  # funding for delisted pairs
XS_RUNS = _root("LDP_XS_RUNS", "xs_runs")                               # banked cross-sectional run directories
