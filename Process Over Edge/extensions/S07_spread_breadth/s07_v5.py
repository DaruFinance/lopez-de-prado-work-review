#!/usr/bin/env python3
"""
S7a + S7b  --  Spread mean-reversion at breadth  (DIVERSITY v5)
================================================================
BASED ON s07_v4.py — all leak-free machinery preserved exactly:
  ...
  * maxtasksperchild=150 on every Pool (prevents pyarrow hashtable SEGFAULT)

KEY CHANGE vs v4: DIVERSITY-FILTERED SELECTION
  - v4 selected top-60 by raw ADF strength -> 54/60 pairs in one token cohort
  - v5 caps each unique constituent token to at most MAX_TOKEN_APPEARANCES pairs
  - Within token budget: ranked by adf_p (most-negative), tie-break ou_half_life
  - Selection still uses ONLY pre-T* metrics (no lookahead introduced)
  - Reports token/cohort distribution of the resulting book
  - Aim: genuine breadth across the 6,817 cointegrated pairs

DO NOT MODIFY s07_v4.py -- this is a NEW file.
"""
...
# ── DIVERSITY FILTER: max appearances per unique constituent token ─────────────
# Each token (e.g. "BTC" from "BTCUSDT") may appear in at most this many
# selected pairs (either as sym_a or sym_b).
MAX_TOKEN_APPEARANCES = 2
...
def _eg_worker(args):
    ...
    Identical to v4 -- no changes to selection logic.
    """
...
def _oos_worker(cp):
    ...
    Identical to v4 -- no changes to trading logic.
    """
...
def screen_cointegration_parallel(candidates: list[tuple]) -> tuple[list[dict], list[tuple]]:
    ...
    with Pool(N_WORKERS, maxtasksperchild=150) as pool:
...
# ──────────────────────────────────────────────────────────────────────────────
#  STAGE 3b: DIVERSITY-FILTERED SELECTION  (KEY CHANGE vs v4)
#  Selection uses ONLY pre-T* metrics (no OOS data).
# ──────────────────────────────────────────────────────────────────────────────

def select_diverse_top_n(coint_pairs: list[dict], n: int,
                         max_token_appearances: int = MAX_TOKEN_APPEARANCES) -> tuple[list[dict], dict]:
    """
    Select up to n cointegrated pairs with diversity filter:
      - Sort all 6,817+ pairs by adf_p (ascending = most negative ADF stat first)
      - Greedily add pairs subject to: each unique base token may appear
        in at most max_token_appearances selected pairs (counting both legs)
      - Tie-break: shortest ou_half_life

    Returns (selected_pairs, diversity_report).
    All selection criteria use pre-T* metrics only.
    """
    print(f"\n=== STAGE 3b: Diversity-filtered selection "
          f"(max {max_token_appearances} appearances/token, target={n}) ===")
    print(f"  Input: {len(coint_pairs)} cointegrated pairs")

    # Sort by adf_p ascending (most-negative first), tie-break ou_half_life ascending
    ranked = sorted(coint_pairs,
                    key=lambda cp: (cp.get("adf_p", 1.0), cp.get("ou_half_life", 1e9)))

    token_counts: dict[str, int] = {}
    selected: list[dict] = []
    rejected_by_token: int = 0
    ...
    for cp in ranked:
        ...
        ta = _token(cp["sym_a"])
        tb = _token(cp["sym_b"])
        ca = token_counts.get(ta, 0)
        cb = token_counts.get(tb, 0)
        if ca >= max_token_appearances or cb >= max_token_appearances:
            rejected_by_token += 1
            continue
        selected.append(cp)
        token_counts[ta] = ca + 1
        token_counts[tb] = cb + 1

    # Diversity report
    n_unique_tokens = len(token_counts)
    token_dist = dict(sorted(token_counts.items(), key=lambda x: -x[1]))
    top_tokens = list(token_dist.items())[:20]

    adf_vals = [cp.get("adf_p", np.nan) for cp in selected]
    hl_vals  = [cp.get("ou_half_life", np.nan) for cp in selected]

    print(f"  Selected: {len(selected)} pairs from {len(ranked)} ranked candidates")
    print(f"  Rejected by token cap: {rejected_by_token}")
    print(f"  Unique constituent tokens: {n_unique_tokens}")
    print(f"  Token appearances (top-20): {top_tokens}")
    print(f"  ADF p range: "
          f"{np.nanmin(adf_vals):.4e} to {np.nanmax(adf_vals):.4e}")
    print(f"  OU half-life range: "
          f"{np.nanmin(hl_vals):.1f} to {np.nanmax(hl_vals):.1f} bars")

    diversity_report = dict(
        n_input_coint=len(coint_pairs),
        n_selected=len(selected),
        n_rejected_token_cap=rejected_by_token,
        max_token_appearances=max_token_appearances,
        n_unique_tokens=n_unique_tokens,
        token_distribution=token_dist,
        top_tokens_by_appearances=top_tokens,
        adf_p_min=float(np.nanmin(adf_vals)) if adf_vals else np.nan,
        adf_p_max=float(np.nanmax(adf_vals)) if adf_vals else np.nan,
        ou_hl_min=float(np.nanmin(hl_vals)) if hl_vals else np.nan,
        ou_hl_max=float(np.nanmax(hl_vals)) if hl_vals else np.nan,
    )

    return selected, diversity_report


# ──────────────────────────────────────────────────────────────────────────────
#  STAGE 4: Hard no-lookahead assertion (runs only on the selected book; fast)
# ──────────────────────────────────────────────────────────────────────────────
...
def run_oos_parallel(coint_pairs: list[dict]) -> list[dict]:
    """
    Parallel OOS trading over selected pairs using Pool(N_WORKERS).
    """
...
def permutation_null(pair_results: list[dict],
    ...
    print(f"  Deflation note: null pairs are non-cointegrated but SAME "
          f"pt/sl grid search runs on top selection. "
...
# ──────────────────────────────────────────────────────────────────────────────
#  STAGE 9: Sign-consistency (disjoint halves)
#  v5: split on first letter of sym_a (A-M vs N-Z), same as v4
# ──────────────────────────────────────────────────────────────────────────────
...
def sign_consistency(pair_results: list[dict]) -> tuple:
    ...
    print(f"\n=== STAGE 9: Sign-consistency (diverse top-{TOP_N}) ===")
...
def make_figures(pair_results, book_r, null_srs, sr_obs, tail, diversity_report):
    try:
        import matplotlib
        matplotlib.use("Agg")
        ...
        axes[0].plot(np.cumsum(book_r), lw=1.0, color="#d7191c", label=f"diverse-{TOP_N}")
        axes[0].axhline(0, color="#888", lw=0.7, ls="--")
        axes[0].set_title(f"S7a: Diverse-{TOP_N} book equity (inv-vol, net 28bp/RT)")
        ...
        # Panel 3: Tail guard per quarter
        ...
        if qs:
            ...
            axes[2].set_title(f"Per-quarter RRR (diverse-{TOP_N} tail-guard)")
            axes[2].legend(fontsize=8)

        # Panel 4: Token distribution
        top_toks = diversity_report.get("top_tokens_by_appearances", [])[:20]
        if top_toks:
            tok_names = [t for t, _ in top_toks]
            tok_counts = [c for _, c in top_toks]
            axes[3].barh(tok_names[::-1], tok_counts[::-1], color="#4393c3", edgecolor="white")
            axes[3].axvline(MAX_TOKEN_APPEARANCES, color="#d7191c", lw=1.5, ls="--",
                            label=f"cap={MAX_TOKEN_APPEARANCES}")
            axes[3].set_title(f"Token appearances in diverse book\n"
                              f"({diversity_report.get('n_unique_tokens',0)} unique tokens)")
            axes[3].legend(fontsize=7)
            axes[3].set_xlabel("# pairs in book")

        fig.suptitle(
            f"S7v5: Diverse-{TOP_N} spread MR book — token-capped "
            f"(T*={T_STAR.date()}, max {MAX_TOKEN_APPEARANCES} appearances/token)",
            fontsize=11)
        fig.tight_layout()
        out_fig = os.path.join(OUT, "book_equity_v5.png")
...
def main():
    ...
    print("=" * 72)
    print("S7a + S7b -- Spread mean-reversion (DIVERSITY v5)")
    print(f"T* = {T_STAR}  (global split; selection uses ONLY pre-T* data)")
    print(f"DIVERSITY FILTER: max {MAX_TOKEN_APPEARANCES} appearances per token")
    print(f"OOS: post-T* only. Workers: {N_WORKERS}. Target book: ~{TOP_N} pairs.")
    ...
    univ = build_universe()
    print(f"\n  Universe: {len(univ)} symbols | {(time.perf_counter()-t0):.1f}s | "
          f"RAM {_ram_gb():.2f} GB")
    ...
    print(f"\n  Candidates: {len(candidates)} | {(time.perf_counter()-t0):.1f}s | "
          f"RAM {_ram_gb():.2f} GB")
    ...
    if n_coint == 0:
        ...
        verdict_path = os.path.join(OUT, "s07_v5_verdict.json")
        with open(verdict_path, "w") as f:
            json.dump(dict(verdict_s7a="FAIL", verdict_s7b="FAIL",
                           n_coint=0, reason="no cointegrated pairs"), f, indent=2)
        return

    # Heartbeat: report token concentration of the raw coint pool
    t0_tok = time.perf_counter()
    raw_token_counts: dict[str, int] = {}
    for cp in coint_pairs:
        for tok in [_token(cp["sym_a"]), _token(cp["sym_b"])]:
            raw_token_counts[tok] = raw_token_counts.get(tok, 0) + 1
    top_raw_toks = sorted(raw_token_counts.items(), key=lambda x: -x[1])[:10]
    print(f"\n  Raw coint pool token concentration (top-10): {top_raw_toks}")
    print(f"  (v4 concentrated 54:6 in one cohort -- v5 caps at {MAX_TOKEN_APPEARANCES}/token)")

    # ── KEY CHANGE: DIVERSITY-FILTERED SELECTION (replaces top-N by raw ADF) ──
    diverse_book, diversity_report = select_diverse_top_n(coint_pairs, TOP_N, MAX_TOKEN_APPEARANCES)
    n_diverse = len(diverse_book)
    print(f"\n  Diverse book: {n_diverse} pairs from {n_coint} cointegrated "
          f"(token-capped at {MAX_TOKEN_APPEARANCES}; pre-T* only)")

    # Free all non-selected coint data from memory
    del coint_pairs; gc.collect()

    if n_diverse == 0:
        print("Diversity filter selected 0 pairs. FAIL.")
        verdict_path = os.path.join(OUT, "s07_v5_verdict.json")
        with open(verdict_path, "w") as f:
            json.dump(dict(verdict_s7a="FAIL", verdict_s7b="FAIL",
                           n_diverse=0, reason="diversity filter yielded no pairs"), f, indent=2)
        return

    # Stage 4: No-lookahead assertion
    t0 = time.perf_counter()
    assert_no_lookahead(diverse_book)
    print(f"  Assertion: {(time.perf_counter()-t0):.1f}s")

    # Stage 5: OOS trading (PARALLEL)
    t0 = time.perf_counter()
    pair_results = run_oos_parallel(diverse_book)
    ...
    # Report OOS token distribution
    oos_token_counts: dict[str, int] = {}
    for pr in pair_results:
        for tok in [_token(pr["sym_a"]), _token(pr["sym_b"])]:
            oos_token_counts[tok] = oos_token_counts.get(tok, 0) + 1
    top_oos_toks = sorted(oos_token_counts.items(), key=lambda x: -x[1])[:20]
    n_unique_oos = len(oos_token_counts)
    print(f"\n  OOS book token distribution: {n_unique_oos} unique tokens")
    print(f"  Top-20 tokens: {top_oos_toks}")

    # Stage 6: Portfolio book (inv-vol)
    print(f"\n=== STAGE 6: Portfolio book (diverse-{n_diverse} inv-vol) ===")
    book_r, book_meta = build_book(pair_results, sizing="inv_vol")

    sc = score_series(book_r) if len(book_r) > 0 else {}
    print(f"  Diverse book: SR_ann={sc.get('sr_ann',0):.3f} | "
    ...
    ledger_path = os.path.join(OUT, "s07_v5_trades_diverse.parquet")
    ...
    for pr in pair_results:
        ...
        pair_rows.append(dict(
            ...
            token_a=_token(pr["sym_a"]), token_b=_token(pr["sym_b"]),
    ...
    pair_parquet = os.path.join(OUT, "s07_v5_pair_table.parquet")
    ...
    p_val, null_srs, sr_obs_perm = permutation_null(
        pair_results, null_cache, N_PERM,
        label=f"[diverse-{n_diverse}] ", rng_seed=42)
    ...
    # Stage 10: S7b sizing comparison
    print(f"\n=== STAGE 10: S7b -- sizing comparison (diverse-{n_diverse}) ===")
    ...
    # ── Honest characterization note ──────────────────────────────────────────
    if not tail["passes"]:
        characterization_note = (
            "FAIL: the diversified spread book's OOS Sharpe does not survive the "
            "tail-guard (ex-worst-quarter RRR <= 1.0). "
            "The high in-sample ADF significance is a grid-corner and/or non-persistence "
            "artifact: OU mean-reversion fitted on pre-T* data is not preserved OOS across "
            "all quarters. Diversification (token cap) does not rescue the tail-guard because "
            "the failure is structural (non-stationarity of the spread post-T*), not a "
            "concentration artifact. S7 is demoted to characterization-negative."
        )
    elif not crit["sign_consistent"]:
        characterization_note = (
            "PARTIAL: diversification fixed the concentration problem but "
            "sign-consistency still fails. One alphabetic half of the book is not "
            "net-positive. The book's positive aggregate result is not uniform across "
            "the universe."
        )
    elif not crit["deflated_sig"]:
        characterization_note = (
            "PARTIAL: tail-guard and sign-consistency pass but the permutation null "
            "is not significant after deflation. Result does not clear the bar."
        )
    else:
        characterization_note = (
            "PASS: diversified book clears net-of-cost deflation, tail-guard "
            "(ex-worst-quarter RRR>1), and sign-consistency."
        )
    ...
    verdict = dict(
        ...
        n_diverse_selected         = n_diverse,
        n_pairs_oos_ok             = len(pair_results),
        # Diversity
        diversity_report           = diversity_report,
        oos_token_distribution     = dict(sorted(oos_token_counts.items(), key=lambda x: -x[1])),
        n_unique_oos_tokens        = n_unique_oos,
        # Diverse book
        diverse_book = dict(
            n_pairs             = len(pair_results),
            selection_criterion = (
                f"most-negative ADF stat (smallest adf_p); "
                f"token cap: max {MAX_TOKEN_APPEARANCES} appearances per unique base token; "
                f"tie-break: shortest OU half-life; pre-T* only; selection BEFORE OOS trading"
            ),
        ...
        characterization_note      = characterization_note,
        # Run metadata
        ...
        n_grid_cells      = int(len(PT_GRID) * len(SL_GRID)),
        artifacts = dict(
            ledger_diverse = ledger_path,
            pairs          = pair_parquet,
            figure         = fig_path,
            verdict        = os.path.join(OUT, "s07_v5_verdict.json"),
        ),
    )
    ...
    # ── Final summary ────────────────────────────────────────────────────────
    print("\n" + "=" * 72)
    print("FINAL VERDICT (S7v5 -- DIVERSITY-FILTERED BOOK)")
    print("=" * 72)
    ...
    print(f"  Diverse selected    : {n_diverse} "
          f"(token cap={MAX_TOKEN_APPEARANCES}; {diversity_report['n_unique_tokens']} unique tokens)")
    print(f"  OOS ok              : {len(pair_results)} "
          f"({n_unique_oos} unique tokens in OOS book)")
    print(f"")
    print(f"  -- DIVERSE BOOK --")
    ...
    print(f"  Grid interior       : {grid_chk['optimum_moved_interior']} "
          f"(median pt*={grid_chk['median_pt_star']:.3f})")
    print(f"  Criterion           : {crit}")
    print(f"  S7a                 : {verdict_s7a}")
    print(f"  S7b                 : {verdict_s7b}  "
          f"(inv_vol RRR_ex={sz_results['inv_vol']['rrr_ex_worst']:.3f} vs "
          f"equal={sz_results['equal']['rrr_ex_worst']:.3f})")
    print(f"")
    print(f"  Token distribution  : {top_oos_toks[:10]}")
    print(f"")
    print(f"  CHARACTERIZATION: {characterization_note}")
    print(f"")
