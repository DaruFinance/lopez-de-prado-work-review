...
- **Corpus inputs**: the at-scale per-strategy daily PnL corpora are produced by a separate data
  pipeline (costed, causal, intrabar OHLC exits; equity and FX friction models verified bit-identical
  between a Numba kernel and a NumPy reference). They are not part of this repository; the study scripts
  here read the resulting per-strategy daily PnL from the `LDP_PNL_DAILY` root.
...
- **Rerun:** `python3 lib/overfit.py` (self-tests); with the per-strategy daily PnL corpora present
  under `LDP_PNL_DAILY`, run `LDP_MARKET={crypto,equity,fx} python3 scripts/run_corpus_overfit.py`
  then `python3 scripts/build_cross_market.py` (the corpus DSR / PBO / effective-N analysis);
  `python3 scripts/run_overfit_at_scale.py` regenerates the per-instrument overfitting figures
  end-to-end from the raw price roots (the §3-4 cross-check). Data: crypto perpetual 1m, Algoseek
