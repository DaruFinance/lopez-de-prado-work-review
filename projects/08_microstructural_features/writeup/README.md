...
**Status: DONE.** Full multi-market run plus a deepening pass (per-estimator attribution + a clean
...
- **Deepening parallelism.** The deepening's single-feature sweep is many tiny fits, so it parallelises
...
The initial cross-market comparison confounds the data tier with the instrument. The clean
...
bash run_full.sh                                   # full multi-market initial run
python3 scripts/deepen_microstructure.py           # deepening: per-estimator + tier ablation
