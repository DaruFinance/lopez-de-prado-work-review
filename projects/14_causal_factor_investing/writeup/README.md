...
## 7b. Climbing toward L6 - a perpetual-listing natural experiment

Sections 5 and 6 stop at Level 3: a factor that survives two independent
observational confounders, but with no defended graph (L4), no interventional
evidence (L6), and no cross-market causal replication (L7). This section adds the
missing interventional flavour with a **quasi-natural experiment**, and applies it
to the program's lone L3 survivor - **short-term reversal**.

**The experiment.** The event is a token gaining a Binance USD-M **perpetual
futures listing**. A listing is an exogenous structural shock to the asset's
market microstructure: it adds leverage, short access, funding-rate mechanics, and
a broad speculative participant base. Crucially, the listing **timing is set by the
exchange, not by the reversal effect we are testing**, so it is exogenous to the
factor - the defining property of a natural experiment, and the property pure
observational conditioning (L1 to L3) can never supply.

**Why reversal, and the pre-registered direction.** Short-term reversal is widely
understood as a microstructure / liquidity-provision effect, so it makes a sharp,
falsifiable, directional prediction: if reversal is structural microstructure, its
strength should shift **discontinuously** when the microstructure changes at the
listing - leveraged speculators bring overreaction and forced-liquidation mean
reversion. This is a genuine *ex-ante* hypothesis about the sign of the effect, not
a fitted result.

**Design (event-time aligned, no look-ahead).** For each of **170 qualifying
listings** the pre-window is the token's own **spot** daily returns before the
listing and the post-window is its **perpetual** daily returns after (same
underlying asset on both sides; the instrument switch *is* the treatment). The
reversal factor is computed within each side separately with the same lagged
5-day definition as the main study, so no factor value straddles the event
boundary or uses future data. We estimate (i) the pooled panel reversal slope and
*t* pre vs post, plus a pre/post interaction term - a **regression-discontinuity**
in the factor effect; and (ii) the event-time cumulative abnormal return of a
continuation sleeve, net of each asset's own pre-listing baseline, with a
bootstrap CI over events. A **control** set of ten established majors (BTC, ETH,
BNB, ...) with **no** listing event, cut at the same calendar dates, serves as a
placebo / parallel-trends check.

**Result** (`tables/event_study.csv`, `tables/event_study.md`,
`figures/fig_event_study.png`):

| group | window | reversal slope | *t* | n |
|---|---|---|---|---|
| treated | pre (spot) | -0.0008 | **-0.50** | 8,671 |
| treated | post (perp) | -0.0050 | **-6.49** | 9,350 |
| treated | post - pre (RD interaction) | -0.0042 | **-2.55** | 18,021 |
| control | pre | -0.0025 | -3.02 | 8,738 |
| control | post | -0.0019 | -3.28 | 9,350 |
| control | post - pre (RD interaction) | +0.0006 | **+0.58** | 18,088 |

- In the **treated** assets the reversal effect is **absent before listing**
 (*t* = -0.50) and **strongly present after** (*t* = -6.49); the discontinuity is
 significant (interaction *t* = **-2.55**). The more-negative post slope means
 recent winners reverse harder once the perp exists.
- In the **control** majors the reversal effect is similar pre and post and the
 discontinuity is **flat** (interaction *t* = +0.58). The jump is tied to the
 event, not to the calendar window - the placebo passes.
- The event-time **continuation sleeve** (which loses when reversal pays) earns a
 cumulative abnormal return of **-14.9%** over the 30 post-listing days, net of
 each asset's own pre-listing baseline (bootstrap two-sided *p* = **0.021**). A
 negative continuation payoff is the reversal effect strengthening - the same
 direction as the regression discontinuity.

**Honest caveat.** The discontinuity is a **pooled-panel average**: asset by asset,
the post-listing reversal |*t*| exceeds the pre-listing |*t*| in only 48% of
events. So this is evidence about the *typical* newly-listed token's
microstructure, not a per-asset law, and the abnormal-return CI band is wide
(it stays below zero throughout but only the cumulative endpoint is comfortably
significant).

**Where this lands on the hierarchy.** This is the first piece of evidence in the
program that is **not purely observational conditioning**. The treatment (perp
listing) is exogenous and the control group rules out the obvious calendar
confound, so it reaches **L6-flavoured (interventional / natural-experiment)
evidence** for a *structural* reading of reversal: an exogenous change in market
microstructure produces a discontinuous, placebo-controlled change in the factor's
effect. It **strengthens** the claim that reversal is a microstructure phenomenon
(and correspondingly **weakens** any reading of it as a defended risk premium - 
the effect is manufactured by the trading structure, not compensated risk). It is
not a clean randomized intervention (listing is exchange-chosen, possibly
correlated with liquidity/attention, which a fuller design would instrument or
match on), and it does **not** rescue the factor at L5: the deflated-Sharpe verdict
in Section 6 is unchanged, and a microstructure effect that flips on at listing is
a cost-and-capacity story, not a tradeable premium. So the corrected scorecard
reads: reversal clears L1 to L3, now gains **partial L6** support for a *structural*
(microstructure) interpretation, yet still fails L4 (no formal defended SEM), L5
(deflation), and L7 (no cross-market causal replication). The chain of evidence is
exactly as fragile as Section 6 concluded - the natural experiment sharpens *what*
the surviving signal is (microstructure, not premium) without turning it into a
risk factor.
...
python3 scripts/run_event_study.py         # natural-experiment extension (~13 s)
python3 scripts/run_event_study.py --smoke # tiny 1-core check (12 events)
```
...
- Tables: `tables/mc_structures.csv`, `tables/mc_sweep.csv`,
  ...
  `tables/event_study.csv`, `tables/event_study.md`
- Figures: `figures/fig1_mc_structures.png` (coefficient distributions),
  ...
  `figures/fig_event_study.png` (event-time abnormal return + RD discontinuity)
