...
## 7. Imbalance and run bars: the advanced tier does not help here

The bars above (tick, volume, dollar) sample on the *magnitude* of activity. LdP also proposes a
harder tier that samples on the *direction* of order flow: **imbalance bars** close when cumulative
signed flow breaks an adaptive expectation, and **run bars** close when a one-sided run of buys or
sells breaks its expectation. The expectation is tracked with a causal EWMA built only from
already-closed bars, so there is no look-ahead in the trigger. We built all four, matched to the
same near-daily frequency as the threshold bars, on three representative perps (BTCUSDT, ETHUSDT,
SOLUSDT). Signed flow uses the tick rule for direction and the observed taker buy/sell split for
signed volume. Results in `tables/imbalance_run_bars.md` and `figures/fig_imbalance_run_bars.png`:

| bar | median excess kurtosis | median \|skew\| |
|---|---|---|
| time | 3.56 | 0.07 |
| tick | **1.07** | 0.32 |
| volume | 1.94 | 0.26 |
| dollar | **1.07** | 0.15 |
| tick-imbalance | 11.32 | 0.01 |
| volume-imbalance | 6.46 | 0.63 |
| tick-run | 3.88 | 0.05 |
| volume-run | 2.90 | 0.21 |

**Honest finding: on this data the advanced bars make the tails worse, not better.** Every
imbalance and run bar has *higher* excess kurtosis than the simple tick, volume, and dollar bars at
matched frequency, and the two imbalance bars (11.3 and 6.5) are fatter-tailed than even the time-bar
control (3.56). They do not Gaussianize returns further; they de-Gaussianize them. The mechanism is
straightforward: sampling on a directional-flow trigger deliberately *waits for* and then *cuts on*
one-sided bursts, so each bar tends to span a directional move and the close-to-close return inherits
that move. Imbalance is, by construction, correlated with return magnitude, so conditioning bar
boundaries on it concentrates the moves rather than spreading them. The run bars are milder (they
track one side at a time) but still lose to the magnitude-clock bars. The only thing that improves is
skew for the tick-direction variants, which is an artifact of the symmetric tick rule, not a tail
benefit.

This is a negative result, and it is the useful kind: for the distributional goal that motivates
information bars in the first place, returns closer to IID-Gaussian, **the simple dollar or volume
bar is the right tool, and the advanced imbalance/run machinery is a step backward.** Imbalance and
run bars may still earn their keep as *features* (a bar that fires on a flow burst is an event marker
a model can use), but they should not be the return-sampling clock. We verified the fast bar builder
against a line-by-line reference implementation on a held-out slice (identical bar boundaries, maximum
absolute difference zero), so the result is a property of the method, not of the implementation.

## 8. Notes, opinions & extensions
...
- **Rerun:** `python3 scripts/run_bars_study_clean.py` (crypto), `run_equities_bars.py` (equities),
  `run_forex_bars.py` (forex), `run_granularity_study.py` (granularity),
  `run_imbalance_run_bars.py` (imbalance/run bars), then `make_final_figure.py`.
